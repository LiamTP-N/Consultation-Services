...
Each "rfps" record has this shape:

  {
    ...
    "applied":   {  # OPTIONAL - present only if we applied for this tender
      "date":    "YYYY-MM-DD when proposal submitted",
      "outcome": "pending | won | lost",
      "note":    "Free-text proposal ref or note"
    }
  }

PRESERVING YOUR MANUAL EDITS
----------------------------
You can hand-edit rfps.json between scraper runs. Four things
the scraper preserves across runs (it does NOT overwrite them):
  ...
  4. The `applied` object on scraped entries (set when you tick
     the "applied for" box on the dashboard). Independent of
     status; preserved across runs by stable id.
...
def merge_with_existing(scraped):
    ...
    for r in existing.get("rfps", []):
        ...
        if r.get("applied") and isinstance(r["applied"], dict) and r["applied"].get("date"):
            applied_overrides[r["id"]] = r["applied"]
    ...
    for r in scraped:
        ...
        if r["id"] in applied_overrides:
            r["applied"] = applied_overrides[r["id"]]
    ...
    return {
        ...
        "notes": (
            ...
            "'closed', any notes, and the 'applied' object are preserved across runs by stable id."
        ),
        "rfps": manual + scraped,
