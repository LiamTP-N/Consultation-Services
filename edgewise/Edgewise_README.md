...
Every day at 10:30 UTC:
  ...
  -> It visits 26 public procurement websites
...
| Doffin (Norway) | JSON API | Norway (sub-threshold) |
...
### One-off: Doffin (Norway) API key setup

The Doffin scraper needs a free Azure-managed API key to call the Norwegian procurement API. Without the key the scraper logs a single warning and skips that source - the rest of the daily run is unaffected.

To enable Doffin:

1. Go to `https://dof-notices-prod-api.developer.azure-api.net/` and click **Sign up** (top right). Use the Edgewise email of your choice.
2. Once signed in, go to **Products**, pick **Public** (or whichever non-paid product is offered), and click **Subscribe**.
3. Open **Profile** → copy the **Primary key** value.
4. In the GitHub repo go to **Settings → Secrets and variables → Actions → New repository secret**.
5. Name: `DOFFIN_API_KEY`. Value: paste the key. Click **Add secret**.
6. Trigger a fresh scrape via the green **Run scrape now** button on the page. Doffin entries will start appearing on the next run.

The key is read by the scraper from the `DOFFIN_API_KEY` environment variable; it never appears in any committed file. If the key is ever revoked or rotated, repeat steps 3-5.
